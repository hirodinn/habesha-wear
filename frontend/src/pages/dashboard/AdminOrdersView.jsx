...
const AdminOrdersView = () => {
  ...
  const handleDeleteOrder = async (order) => {
    if (!confirm(`Are you sure you want to delete order "${order._id}"? This action is permanent.`)) {
      return;
    }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-3">
                            ...
                            <p className="text-sm text-(--text-main)">
                              <span className="font-semibold">Customer:</span> {getOrderUserName(order)}
                            </p>
                            <p className="text-sm text-(--text-main)">
                              <span className="font-semibold">User ID:</span> {getOrderUserId(order)}
                            </p>
                            <p className="text-sm text-(--text-main)">
                              <span className="font-semibold">Total:</span> {Number(order.totalAmount || 0).toLocaleString()} Birr
                            </p>
                            <p className="text-sm text-(--text-main)">
                              <span className="font-semibold">Ordered:</span> {new Date(order.orderDate || order.createdAt || Date.now()).toLocaleString()}
                            </p>
                            {order.shippingAddress && (
                              <>
                                ...
                                <p className="text-sm text-(--text-main)"><span className="font-semibold">Name:</span> {shippingAddress?.name || "N/A"}</p>
                                <p className="text-sm text-(--text-main)"><span className="font-semibold">Phone:</span> {shippingAddress?.phone || "N/A"}</p>
                                <p className="text-sm text-(--text-main)"><span className="font-semibold">Email:</span> {shippingAddress?.email || "N/A"}</p>
                                <p className="text-sm text-(--text-main)"><span className="font-semibold">Address:</span> {shippingAddress?.addressLine || "N/A"}</p>
                                <p className="text-sm text-(--text-main)"><span className="font-semibold">City:</span> {shippingAddress?.city || "N/A"}</p>
                                <p className="text-sm text-(--text-main)"><span className="font-semibold">Postal Code:</span> {shippingAddress?.postalCode || "N/A"}</p>
                              </>
                            )}
                          </div>
                            ...
                            {order.products?.length ? (
                              <div className="space-y-2">
                                {order.products.map((item, idx) => {
                                  ...
                                  return (
                                  <div key={idx} className="p-3 rounded-lg border border-(--border-color) bg-(--bg-main) flex items-center gap-3">
                                    {image ? (
                                      <img src={image} alt={name} className="w-10 h-10 rounded-lg object-cover border border-(--border-color)" />
                                    ) : (
                                      <div className="w-10 h-10 rounded-lg border border-(--border-color) bg-(--bg-card) flex items-center justify-center text-[10px] text-(--text-secondary)">N/A</div>
                                    )}
                                    <div className="min-w-0">
                                      <p className="text-sm text-(--text-main) font-semibold truncate">{name}</p>
                                      <p className="text-xs text-(--text-secondary)">Quantity: <span className="font-semibold">{item.quantity}</span></p>
                                      <p className="text-[10px] text-(--text-secondary) font-mono truncate">Product ID: {productId}</p>
                                    </div>
                                  </div>
                                  );
                                })}
