...
const AdminCartsView = () => {
  ...
  const handleDeleteCart = async (cart) => {
    if (!confirm(`Are you sure you want to delete cart "${cart._id}"? This action is permanent.`)) {
      return;
    }
            ...
            <tbody className="divide-y divide-(--border-color)">
              {filteredCarts.map((cart) => (
                <Fragment key={cart._id}>
                  <tr
                    onDoubleClick={() =>
                      setExpandedCartId((prev) => (prev === cart._id ? null : cart._id))
                    }
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-mono text-xs text-(--text-main)">
                      <div className="text-sm font-medium text-(--text-main)">{getCartUserName(cart)}</div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-(--text-secondary) font-medium italic">Items</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <tr className="bg-[var(--color-burgundy)]/5 animate-fade-in">
                      <td colSpan="5" className="px-6 py-6 border-b border-(--border-color)">
                        <div className="space-y-4">
                          <div>
                            <h4 className="text-xs font-bold uppercase tracking-wider text-(--text-secondary)">Cart Details</h4>
                            <p className="text-sm text-(--text-main) mt-2"><span className="font-semibold">Customer:</span> {getCartUserName(cart)}</p>
                            <p className="text-sm text-(--text-main)"><span className="font-semibold">User ID:</span> {getCartUserId(cart)}</p>
                          </div>
                          <div>
                            <h4 className="text-xs font-bold uppercase tracking-wider text-(--text-secondary) mb-2">Items ({cart.products?.length || 0})</h4>
                                ...
                                {cart.products.map((item, idx) => {
                                  ...
                                  <div key={idx} className="p-3 rounded-lg border border-(--border-color) bg-(--bg-main) flex items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                      <p className="text-sm text-(--text-main) truncate"><span className="font-semibold">Product:</span> {name}</p>
